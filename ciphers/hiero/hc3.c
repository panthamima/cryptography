/*****************************************************************

HIEROCRYPT-3 block cipher

Implemented by Alexander Myasnikow

Block: 128 bit

Key: 256 bit

Web: www.darksoftware.narod.ru

*****************************************************************/

#include <stdio.h>
#include <stdlib.h>

typedef unsigned char u8;
typedef unsigned long int u32;

#define primitiveGF8 0x163
#define MAX_ROUND (8)

typedef u8 HC3_KS[MAX_ROUND + 1][8][4];

#include <stdlib. h>

/*
* constant parameters
*/

u8 sbox[256] =
{
  

};

u8 isbox[256] =
{
  
};

u8 hconst[4][4] =
{
  0x5a, 0x82, 0x79, 0x99,
  0x6e, 0xd9, 0xeb, 0xa1,
  0x8f, 0x1b, 0xbc, 0xdc,
  0xca, 0x62, 0xc1, 0xd6
};
int gindex[6][2] =
{
  3, 0, 2, 1, 1, 3, 0, 2, 2, 3, 1, 0
};
int kconst[3][10] =
{
  {0, 1, 2, 3, 3, 2, 1, -1, -1, -1},
  {1, 0, 3, 2, 2, 3, 0, 1, -1, -1},
  {4, 0, 2, 1, 3, 3, 1, 2, 0, -1}
};

u8 mds[4][4] =
{
  {0xc4, 0x65, 0xc8, 0x8b},
  {0x8b, 0xc4, 0x65, 0xc8},
  {0xc8, 0x8b, 0xc4, 0x65},
  {0x65, 0xc8, 0x8b, 0xc4}
};

u8 mds_inv[4][4] =
{
  {0x82, 0xc4, 0x34, 0xf6},
  {0xf6, 0x82, 0xc4, 0x34},
  {0x34, 0xf6, 0x82, 0xc4},
  {0xc4, 0x34, 0xf6, 0x82}
};

int mdsh[4][4] =
{
  {0x5, 0x5, 0xa, 0xe},
  {0xe, 0x5, 0x5, 0xa},
  {0xa, 0xe, 0x5, 0x5},
  {0x5, 0xa, 0xe, 0x5}
};

int mdsh_inv[4][4] =
{
  {0xb, 0xe, 0xe, 0x6},
  {0x6, 0xb, 0xe, 0xe},
  {0xe, 0x6, 0xb, 0xe},
  {0xe, 0xe, 0x6, 0xb}
};


HC3_KS ks;
HC3_KS dks;


int
poly32_deg (u32 a)
{
  int n = -1;
  for (; a; a >>= 1)
    n++;
  return n;
}


u32
poly32_mul (u32 a, u32 b)
{
  u32 c = 0;

  for (; b; b >>= 1, a <<= 1)
    if (b & 1)
      c ^= a;

  return c;
}



u32
poly32_mod (u32 a, u32 b)
{
  int da = poly32_deg (a);
  int db = poly32_deg (b);
  u32 t;

  if (da < db)
    return a;

  if (da == db)
    return a ^ b;

  b <<= da - db;

  for (t = 1 << da; da >= db; da--)
    {
      if (a & t)
        a ^= b;
      b >>= 1;
      t >>= 1;
    }
  return a;
}


void
hcrypt_mdsl (u8 * in, u8 * out)
{
  int i, j;
  u32 m;
  for (i = 0; i < 4; i++)
    {
      m = 0;
      for (j = 0; j < 4; j++)
        {
          m ^= poly32_mod (poly32_mul (mds[i][j], in[j]), primitiveGF8);
        }
      out[i] = (u8) m;

    }
}

void
hcrypt_xs (u8 * in, u8 * out, u8 * k1, u8 * k2)
{
  u8 t[4], u[4];
  int i;

  for (i = 0; i < 4; i++)
    u[i] = in[i] ^ k1[i];       /* key xOR */

  for (i = 0; i < 4; i++)
    t[i] = sbox[u[i]];          /* sbox */

  hcrypt_mdsl (t, u);           /* MDS_L */

  for (i = 0; i < 4; i++)
    t[i] = u[i] ^ k2[i];        /*keyxOR */
  for (i = 0; i < 4; i++)
    out[i] = sbox[t[i]];        /* sbox */
}

/*
* op. over GF(2^4)  in MDS_H
*/

void
mdsh_mul (u8 * in, u8 * out, int x)
{
  int i;
  u8 u[4];
  for (i = 0; i < 4; i++)
    u[i] = 0;
  if (x & 1)
    {
      u[0] ^= in[0];
      u[1] ^= in[1];
      u[2] ^= in[2];
      u[3] ^= in[3];
    }
  if (x & 2)
    {
      u[0] ^= in[1];
      u[1] ^= in[2];
      u[2] ^= in[3] ^ in[0];
      u[3] ^= in[0];
    }
  if (x & 4)
    {
      u[0] ^= in[2];
      u[1] ^= in[3] ^ in[0];
      u[2] ^= in[0] ^ in[1];
      u[3] ^= in[1];
    }
  if (x & 8)
    {
      u[0] ^= in[0] ^ in[3];
      u[1] ^= in[1] ^ in[0];
      u[2] ^= in[2] ^ in[1];
      u[3] ^= in[2];
    }
  for (i = 0; i < 4; i++)
    out[i] = u[i];
}


void
hcrypt_mdsh (u8 in[4][4], u8 out[4][4])
{
  int i, j, k;
  u8 tmp[4];
  for (i = 0; i < 4; i++)
    for (j = 0; j < 4; j++)
      out[i][j] = 0;
  for (i = 0; i < 4; i++)
    {
      for (j = 0; j < 4; j++)
        {
          mdsh_mul (&in[j][0], tmp, mdsh[i][j]);
          for (k = 0; k < 4; k++)
            out[i][k] ^= tmp[k];
        }
    }
}


void __stdcall __export
crypt (u8 * in)
{
  u8 t[4][4], u[4][4];
  int i, j, r;
  int n;
  for (r = i = 0; i < 4; i++)
    {
      for (j = 0; j < 4; j++, r++)
        {
          t[i][j] = in[r];
        }

    }

  n = 7;

  for (r = 0; r < n; r++)
    {
      hcrypt_xs (&t[0][0], &u[0][0], &ks[r][0][0], &ks[r][4][0]);
      hcrypt_xs (&t[1][0], &u[1][0], &ks[r][1][0], &ks[r][5][0]);
      hcrypt_xs (&t[2][0], &u[2][0], &ks[r][2][0], &ks[r][6][0]);
      hcrypt_xs (&t[3][0], &u[3][0], &ks[r][3][0], &ks[r][7][0]);
      hcrypt_mdsh (u, t);
    }

  hcrypt_xs (&t[0][0], &u[0][0], &ks[n][0][0], &ks[n][4][0]);
  hcrypt_xs (&t[1][0], &u[1][0], &ks[n][1][0], &ks[n][5][0]);
  hcrypt_xs (&t[2][0], &u[2][0], &ks[n][2][0], &ks[n][6][0]);
  hcrypt_xs (&t[3][0], &u[3][0], &ks[n][3][0], &ks[n][7][0]);

  for (r = i = 0; i < 4; i++)
    {
      for (j = 0; j < 4; j++, r++)
        {
          in[r] = u[i][j] ^ ks[n + 1][i][j];
        }
    }
}


void
hcrypt_imdsl (u8 * in, u8 * out)
{
  int i, j;
  u32 m;
  for (i = 0; i < 4; i++)
    {
      m = 0;
      for (j = 0; j < 4; j++)
        {
          m ^= poly32_mod (poly32_mul (mds_inv[i][j], in[j]), primitiveGF8);
        }
      out[i] = m;
    }
}

void
hcrypt_ixs (u8 * in, u8 * out, u8 * k1, u8 * k2)
{
  u8 t[4], u[4];
  int i;

  for (i = 0; i < 4; i++)
    u[i] = in[i] ^ k1[i];       /* key xOR */

  for (i = 0; i < 4; i++)
    t[i] = isbox[u[i]];         /* sbox */

  hcrypt_imdsl (t, u);          /*MDS_L */

  for (i = 0; i < 4; i++)
    t[i] = u[i] ^ k2[i];        /* key xOR */

  for (i = 0; i < 4; i++)
    out[i] = isbox[t[i]];       /* sbox */
}

void
hcrypt_imdsh (u8 in[4][4], u8 out[4][4])
{
  int i, j, k;
  u8 tmp[4];
  for (i = 0; i < 4; i++)
    for (j = 0; j < 4; j++)
      out[i][j] = 0;
  for (i = 0; i < 4; i++)
    {
      for (j = 0; j < 4; j++)
        {
          mdsh_mul (&in[j][0], tmp, mdsh_inv[i][j]);

          for (k = 0; k < 4; k++)
            out[i][k] ^= tmp[k];

        }
    }
}


void __stdcall __export
decrypt (u8 * in)
{
  u8 t[4][4], u[4][4];
  int i, j, r;
  int n;
  for (r = i = 0; i < 4; i++)
    {
      for (j = 0; j < 4; j++, r++)
        {
          t[i][j] = in[r];
        }
    }
  n = 7;

  for (r = 0; r < n; r++)
    {
      hcrypt_ixs (&t[0][0], &u[0][0], &dks[r][0][0], &dks[r][4][0]);
      hcrypt_ixs (&t[1][0], &u[1][0], &dks[r][1][0], &dks[r][5][0]);
      hcrypt_ixs (&t[2][0], &u[2][0], &dks[r][2][0], &dks[r][6][0]);
      hcrypt_ixs (&t[3][0], &u[3][0], &dks[r][3][0], &dks[r][7][0]);
      hcrypt_imdsh (u, t);
    }

  hcrypt_ixs (&t[0][0], &u[0][0], &dks[r][0][0], &dks[n][4][0]);
  hcrypt_ixs (&t[1][0], &u[1][0], &dks[r][1][0], &dks[n][5][0]);
  hcrypt_ixs (&t[2][0], &u[2][0], &dks[r][2][0], &dks[n][6][0]);
  hcrypt_ixs (&t[3][0], &u[3][0], &dks[r][3][0], &dks[n][7][0]);

  for (r = i = 0; i < 4; i++)
    {
      for (j = 0; j < 4; j++, r++)
        {
          in[r] = u[i][j] ^ dks[n + 1][i][j];

        }
    }
}


void
hcrypt_keyf (u8 * in, u8 * fout, u8 * fkey)
{
  int i;
  /* F in */
  for (i = 0; i < 8; i++)
    fout[i] = sbox[in[i] ^ fkey[i]];

  /* P (32) */

  fout[0] ^= fout[4];
  fout[1] ^= fout[5];
  fout[2] ^= fout[6];
  fout[3] ^= fout[7];
  fout[4] ^= fout[2];
  fout[5] ^= fout[3];
  fout[6] ^= fout[0];
  fout[7] ^= fout[1];
}


void
swap_key (u8 * l, u8 * r)
{
  u8 t;
  int i;
  for (i = 0; i < 8; i++)
    {
      t = l[i];
      l[i] = r[i];
      r[i] = t;
    }
}


void
hcrypt_keyp (u8 k[4][8], u8 kout[8][4], int index)
{
  int i;
  u8 fout[8];
  /* P(32) */
  for (i = 0; i < 4; i++)
    {
      k[2][i] ^= k[3][i];
      k[2][i + 4] ^= k[3][i + 4];
    }
  for (i = 0; i < 4; i++)
    {
      k[3][i] ^= k[2][i + 4];
      k[3][i + 4] ^= k[2][i];
    }

  /* multiple */
  mdsh_mul (&k[2][0], &k[2][0], 5);
  mdsh_mul (&k[2][4], &k[2][4], 0xe);
  mdsh_mul (&k[3][0], &k[3][0], 5);
  mdsh_mul (&k[3][4], &k[3][4], 0xe);

  for (i = 0; i < 4; i++)
    k[2][i] ^= hconst[gindex[index][0]][i];

  for (i = 0; i < 4; i++)
    k[2][i + 4] ^= hconst[gindex[index][1]][i];

  hcrypt_keyf (&k[1][0], fout, &k[2][0]);

  for (i = 0; i < 8; i++)
    k[0][i] ^= fout[i];         /* L xor f(R) */

  for (i = 0; i < 4; i++)
    {
      kout[0][i] = k[0][i];
      kout[1][i] = k[0][i + 4];
    }

  for (i = 0; i < 4; i++)
    {
      kout[2][i] = fout[i] ^ k[2][i];
      kout[3][i] = fout[i + 4] ^ k[2][i + 4];
    }

  for (i = 0; i < 4; i++)
    {
      kout[4][i] = fout[i] ^ k[3][i];
      kout[5][i] = fout[i + 4] ^ k[3][i + 4];
    }

  for (i = 0; i < 4; i++)
    {
      kout[6][i] = k[1][i] ^ k[3][i];
      kout[7][i] = k[1][i + 4] ^ k[3][i + 4];
    }

  swap_key (&k[0][0], &k[1][0]);
}

void
hcrypt_keyc (u8 k[4][8], u8 kout[8][4], int index)
{
  int i;
  u8 fout[8];
  swap_key (&k[0][0], &k[1][0]);
  hcrypt_keyf (&k[1][0], fout, &k[2][0]);

  for (i = 0; i < 8; i++)
    k[0][i] ^= fout[i];

  for (i = 0; i < 4; i++)
    {
      kout[0][i] = k[0][i] ^ k[2][i];
      kout[1][i] = k[0][i + 4] ^ k[2][i + 4];
    }

  for (i = 0; i < 4; i++)
    k[2][i] ^= hconst[gindex[index][0]][i];


  for (i = 0; i < 4; i++)
    k[2][i + 4] ^= hconst[gindex[index][1]][i];

  mdsh_mul (&k[2][0], &k[2][0], 0xb);
  mdsh_mul (&k[2][4], &k[2][4], 0x3);
  mdsh_mul (&k[3][0], &k[3][0], 0xb);
  mdsh_mul (&k[3][4], &k[3][4], 0x3);

  for (i = 0; i < 4; i++)
    {
      kout[2][i] = fout[i] ^ k[2][i];
      kout[3][i] = fout[i + 4] ^ k[2][i + 4];
    }

  for (i = 0; i < 4; i++)
    {
      kout[4][i] = fout[i] ^ k[3][i];
      kout[5][i] = fout[i + 4] ^ k[3][i + 4];
    }

  for (i = 0; i < 4; i++)
    {
      kout[6][i] = k[1][i] ^ k[3][i];
      kout[7][i] = k[1][i + 4] ^ k[3][i + 4];
    }


  for (i = 0; i < 4; i++)
    {
      k[3][i] ^= k[2][i + 4];
      k[3][i + 4] ^= k[2][i];
    }
  for (i = 0; i < 4; i++)
    {
      k[2][i] ^= k[3][i];
      k[2][i + 4] ^= k[3][i + 4];
    }
}


void __stdcall __export
setup (u8 * key)
{
  u8 k[4][8];
  u8 fout[8];
  int i, j, pos, r, n;

  pos = 0;
  for (i = 0; i < 4; i++)
    for (j = 0; j < 8; j++)
      k[i][j] = key[pos++];


  mdsh_mul (&k[2][0], &k[2][0], 5);
  mdsh_mul (&k[2][4], &k[2][4], 0xe);
  mdsh_mul (&k[3][0], &k[3][0], 5);
  mdsh_mul (&k[3][4], &k[3][4], 0xe);

  for (i = 0; i < 4; i++)
    k[2][i] ^= hconst[gindex[5][0]][i];

  for (i = 0; i < 4; i++)
    k[2][i + 4] ^= hconst[gindex[5][1]][i];

  hcrypt_keyf (&k[1][0], fout, &k[2][0]);

  for (i = 0; i < 8; i++)
    k[0][i] ^= fout[i];         /*L+f(R) */

  swap_key (&k[0][0], &k[1][0]);

  for (r = 0; r < 5; r++)
    hcrypt_keyp (&k[0], &ks[r][0], kconst[2][r]);

  for (r = 5; r < 9; r++)
    hcrypt_keyc (&k[0], &ks[r][0], kconst[2][r]);

  r = 8;

  for (j = 0; j < 4; j++)
    {
      for (n = 0; n < 4; n++)
        {
          dks[0][j][n] = ks[r][j][n];
        }
    }

  for (i = 1; i < r; i++)
    {
      for (j = 4; j < 8; j++)
        {
          hcrypt_imdsl (&ks[r - i][j][0], &dks[i - 1][j][0]);
        }
      hcrypt_imdsh (&ks[r - i][0], &dks[i][0]);
    }

  for (j = 4; j < 8; j++)
    {
      hcrypt_imdsl (&ks[0][j][0], &dks[r - 1][j][0]);
    }

  for (j = 0; j < 4; j++)
    {
      for (n = 0; n < 4; n++)
        {
          dks[r][j][n] = ks[0][j][n];
        }
    }



}




#include <string.h>


u32 __stdcall __export
getblocksize ()
{
  return 128;
}

u32 __stdcall __export
getkeysize ()
{
  return 256;
}

void __stdcall __export
getciphername (u8 * p)
{
  strcpy (p, "Hierocrypt-3");
}
